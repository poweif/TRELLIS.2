...
struct Buffer {
    ...
    void extend(size_t size) {
        ...
        if (new_size > capacity) {
            ...
            hipStream_t stream = at::hip::getCurrentHIPStreamMasqueradingAsCUDA().stream();
            CUDA_CHECK(hipMemcpyAsync(new_ptr, ptr, this->size * sizeof(T), hipMemcpyDeviceToDevice, stream));
            CUDA_CHECK(hipStreamSynchronize(stream));
            CUDA_CHECK(hipFree(ptr));
            ptr = new_ptr;
    ...
    void zero() {
        hipStream_t stream = at::hip::getCurrentHIPStreamMasqueradingAsCUDA().stream();
        CUDA_CHECK(hipMemsetAsync(ptr, 0, size * sizeof(T), stream));
    }

    void fill(T val) {
        hipStream_t stream = at::hip::getCurrentHIPStreamMasqueradingAsCUDA().stream();
        std::vector<T> tmp(size, val);
        CUDA_CHECK(hipMemcpyAsync(ptr, tmp.data(), size * sizeof(T), hipMemcpyHostToDevice, stream));
        CUDA_CHECK(hipStreamSynchronize(stream));
    }
};
