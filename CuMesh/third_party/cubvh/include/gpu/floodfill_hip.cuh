...
{  
    ...
    {  
       ...
       hipLaunchKernelGGL(( initLabels), dim3(blocks), dim3(THREADS_PER_BLOCK), 0, 0, d_grid, d_labels, Ntot);  
    ...
    while (h_changed && iteration < MAX_ITERATIONS) {  
        ...
        {  
            int blocks = divUp(Ntot, THREADS_PER_BLOCK);  
           hipLaunchKernelGGL(( hookBatch), dim3(blocks), dim3(THREADS_PER_BLOCK), 0, 0,   
        ...
        if (iteration % 5 == 4 || iteration < 10) {  // Compress more frequently early on
            int blocks = divUp(Ntot, THREADS_PER_BLOCK);  
           hipLaunchKernelGGL(( compress), dim3(blocks), dim3(THREADS_PER_BLOCK), 0, 0, d_labels, Ntot);  
        }
