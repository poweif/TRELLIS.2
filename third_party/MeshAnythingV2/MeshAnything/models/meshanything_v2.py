...
class MeshAnythingV2(nn.Module, PyTorchModelHubMixin,
    ...
    def __init__(self, config={}):
        ...
        # PATCHED for transformers>=5: `use_flash_attention_2=True` is a removed legacy
        # kwarg (config._attn_implementation is already "flash_attention_2" above, which
        # is what modern transformers reads); `to_bettertransformer()` no longer exists
        # on PreTrainedModel (HF's BetterTransformer integration was removed/superseded
        # by native SDPA/flash attention, which we already select via attn_implementation).
