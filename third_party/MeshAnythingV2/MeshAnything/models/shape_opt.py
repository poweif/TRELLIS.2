...
class ShapeOPTDecoder(OPTDecoder):
    ...
    def __init__(self, config: ShapeOPTConfig):
        ...
        # PATCHED for transformers>=5: OPTDecoderLayer now needs layer_idx to index
        # correctly into a shared Cache object; omitting it breaks KV-caching at
        # generate()-time (transformers warns about exactly this at construction).
    ...
    def forward(
        ...
        # print("used my Trans")
        #
        # PATCHED for transformers>=5 (was written against the ~4.39 OPT decoder API).
        # `past_key_values` is now a `Cache` object (not a legacy tuple-of-tuples),
        # `OPTDecoderLayer.forward` now returns a bare hidden_states tensor (not a
        # tuple), and no longer accepts `layer_head_mask`/`past_key_value`/
        # `output_attentions` as named kwargs (they're silently absorbed into
        # **kwargs and ignored, so the old call pattern would produce a
        # tensor-indexed-as-tuple bug rather than a clean crash). `head_mask` and
        ...
        if attention_mask is None:
            mask_seq_length = past_seen_tokens + seq_length
